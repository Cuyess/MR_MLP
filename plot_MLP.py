...
# [Confusion matrix color] Keep the requested blue-purple tone (#5E72E4)
custom_cm = sns.light_palette("#5E72E4", n_colors=100, as_cmap=True)

# [ROC curve color] Nature (NPG) classic palette
roc_colors = {
...
try:
    ...
    print("Error: Data files not found. Ensure roc_curves_data.npy and confusion_matrix_stats.csv are in the current directory.")
    # Create empty structures to keep plotting logic runnable
...
# ==========================================
# 2. Plot function definitions
# ==========================================

def plot_real_roc_with_ci(ax, dataset_key, title_text):
    """
    Plot ROC curves with real shading using a Nature-style palette.
    """
    data_dict = roc_data.get(dataset_key, {})
    
    # Curve config (name, linestyle, linewidth, z-order)
    curves_config = [
        ('TRD vs HC', ':', 1.5, 2),
        ('TRD vs nTRD', '-.', 1.5, 3),
        ('Macro Average', '--', 2.0, 4),
...
def plot_styled_cm_mean_only(ax, dataset_name, title_text):
    """
    Plot confusion matrix: display mean values only.
    """
...
print("Figure 2 has been generated.\nFeatures: confusion matrices show mean values only in blue-purple style; ROC curves use Nature-like colors with real shading.")
